...
class Entradas extends Validator
{
    ...
    */
    ...
    //Primera opción para la gráfica de entradas
    public function firstOption()
    {
        $sql = 'SELECT fecha, count(id_inventario) cantidad
                FROM entrada_inventario INNER JOIN inventario USING(id_inventario)
     ...
     //Segunda opción para la gráfica de entradas
     public function secondOption()
     {
         $sql = 'SELECT nombre, sum(cantidad) cantidad
                FROM entrada_inventario INNER JOIN inventario USING(id_inventario)
                WHERE fecha between ? and ?
                GROUP BY nombre, cantidad ORDER BY cantidad DESC';  
         $params = array($this->fecha1, $this->fecha2);
         return Database::getRows($sql, $params);
     }
}
